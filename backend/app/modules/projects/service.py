...
from app.modules.projects.enums import ProjectStatus, TaskStatus
...
async def create_project(
    ...
    project = Project(**_dump(payload))
    project = await repo.create_project(db, project)
...
async def create_task(
    ...
    task = await repo.create_task(
        db, task, payload.assignee_user_ids, label_ids, checklist
    )
    ...
    if payload.assignee_user_ids:
        await event_bus.publish(
            TaskAssigned(task_id=task.id, assignee_user_ids=payload.assignee_user_ids)
        )
...
async def update_task(db: AsyncSession, task_id: UUID, payload: TaskUpdate) -> TaskRead:
    ...
    update_data = _dump(payload, exclude_unset=True, exclude={"assignee_user_ids"})
    for field, value in update_data.items():
        setattr(task, field, value)

    if payload.assignee_user_ids is not None:
        await repo.set_task_assignees(db, task_id, payload.assignee_user_ids)

    await db.commit()

    if payload.assignee_user_ids:
        await event_bus.publish(
            TaskAssigned(task_id=task_id, assignee_user_ids=payload.assignee_user_ids)
        )
...
async def update_checklist_item(
    ...
    *,
    ...
    if task is None:
        raise NotFoundError("Task", task_id)
    item = next((entry for entry in task.checklist_items if entry.id == checklist_item_id), None)
    if item is None:
...
async def create_project_dependency(
    ...
    edges = [
        (d.predecessor_project_id, d.successor_project_id) for d in existing
    ]
    cycle_path = would_create_cycle(
...
async def get_portfolio_gantt(db: AsyncSession, include_archived: bool = False) -> PortfolioGantt:
    ...
    for project in projects:
        ...
        rows.append(
            PortfolioGanttProject(
                ...
                or project.estimated_completion_date,
    ...
    return PortfolioGantt(
        ...
        links=[
            ...
            if d.predecessor_project_id in visible_ids and d.successor_project_id in visible_ids
        ],
    )
...
async def get_project_timeline(db: AsyncSession, project_id: UUID) -> ProjectTimeline:
    ...
    def _gantt_schedule(t: TaskItem) -> tuple[Optional[date], float]:
        ...
        bar_start = t.start_date or (t.created_at.date() if t.created_at else t.due_date)
